...
**P.S.**
- IP分片时的操作：
0. 计算带分片的IP分组数据部分总长N，检查是否需要分片且是否允许分片
1. 如果不允许分片(DF=1)，向源端返回ICMP报文；否则到2
2. 计算分片后目标链路的数据报长度 n = MTU-20B(IP头) 按8字节取整
3. 对N进行分片，每个分组标识相同，片偏移量为t×n，片偏移字段为 t×n/8(t从0开始)。除最后一个分片外，MF=1
4. 对最后一个分片，MF=0,数据部分按8字节取整,总长度字段为取整之后的长度
